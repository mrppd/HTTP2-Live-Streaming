...
PUSH = 3
QUALITY = 3
AUTO_QUALITY = 0    #1 for yes. 0 for no.
SHOW_OUTPUT = 1     #1 for yes. 0 for no.
ARTIFICIAL_RTT = 0  #only value greater than 100 or equal have an effect
MAX_TIME = 212
...
def intelligentPush(min_quality = 1, max_quality = 7): 
    ...
    while(True):
        ...
        cv2.waitKey(5000)
        if(FRAME_PER_SEC>35):
            if(QUALITY+1>=min_quality and QUALITY+1<=max_quality):
                QUALITY += 1
        elif(FRAME_PER_SEC<30):
            if(QUALITY-1>=min_quality and QUALITY-1<=max_quality):
                QUALITY -= 1     
        """
        QUALITY = int((((DATA_Q.size() - 0) * (max_quality - min_quality)) / float(24*6 - 0)) + min_quality)
...
def showFrame():
    ...
    while(True):
        if(DATA_Q.size()>0):
            ...
            #cv_img = np.frombuffer(data)
            #Display the resulting frame
            #resized_cv_img = cv2.resize(cv_img, (640, 360))
            ...
            if cv2.waitKey(30) & 0xFF == ord('q'):
                cv_img.release()
                #resized_cv_img.release()
                cv2.destroyAllWindows()  
                break
                #print("end")
        else:
            INTERRUPTIONS += 1
            #while(DATA_Q.size()<24*6):
                #Do = 'Nothing'
                #do something
            #cv2.waitKey(6000)
